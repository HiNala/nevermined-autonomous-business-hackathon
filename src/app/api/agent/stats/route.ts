...
export async function GET() {
  ...
  const totalCredits = completed.reduce((sum, e) => sum + Number(e.data.credits ?? 0), 0);
  const uniqueCallers = new Set(completed.map((e) => e.data.caller).filter(Boolean));
  const avgDuration =
    completed.length > 0
      ? Math.round(completed.reduce((sum, e) => sum + Number(e.data.durationMs ?? 0), 0) / completed.length)
      : 0;
